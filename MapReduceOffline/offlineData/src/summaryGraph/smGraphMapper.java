...
public class smGraphMapper extends MapReduceBase
...
//			forYago_sumEdge(values, output, reporter);
	...
	private void forYago2_sumEdge(Text values, OutputCollector<Text, Signature> output, Reporter reporter) throws IOException
	{
		{
			_ii ++;
//			if(_ii % 10000 != 0){
...
//			locID = Integer.parseInt(rdfUnit[3]);
		 ...
		 */
		{
			...
			int _cur_subID = subID, _cur_objID = objID;
			int i_level = 0;
			if(objID >= subNum)//with literal: l_subID_level_objID
			{
				iSig.clear();
				_sb.setLength(0);
				offlineDriver.buildPreSignature(preID, iSig, 1);
				_sb.append("l_").append(subID).append("_")
				   .append(i_level).append("_").append(objID);
				tKey.set(_sb.toString());
				output.collect(tKey, iSig);
				{
					reporter.incrCounter(oReport.ledgeNum, 1);
				}
				return;
			...
			{
				_sb.setLength(0);
				_sb.append("s_").append(_cur_subID).append("_")
				   .append(i_level).append("_").append(_cur_objID);	
				tKey.set(_sb.toString());
				output.collect(tKey, iSig);
				{// for objID -> subID
					_sb.setLength(0);
					_sb.append("o_").append(_cur_objID).append("_")
					.append(i_level).append("_").append(_cur_subID);
					tKey.set(_sb.toString());
					output.collect(tKey, iSig);
				}
				{//get subID father
